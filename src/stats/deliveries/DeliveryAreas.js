...
export default class DeliveryAreas extends React.Component{
    constructor (props){
        super(props)
        this.state = {
            data:[],
    ...
    getDrivers(){
      ...
      tempArr.push(this.Deliveries.state.deliveries[0].pu_location.split(',')[1])
      for (var x = 0; x < this.Deliveries.state.deliveries.length; x++){
        let counter = true
          for (var i = 0; i < tempArr.length; i++){
            if (this.Deliveries.state.deliveries[x].pu_location.split(',')[1] === tempArr[i]){
               counter = false
            }
           
          }
          if (counter)
              tempArr.push(this.Deliveries.state.deliveries[x].pu_location.split(',')[1])
      }
      ...
      countDrivers(drivers){
        ...
        var labels =  new Array()
          for (var i = 0; i < drivers.length; i++){
              ...
              for (var x = 0; x < this.Deliveries.state.deliveries.length; x++){
                  if (drivers[i] === this.Deliveries.state.deliveries[x].pu_location.split(',')[1]){
                    total++;
                  }
              }
              tempArr.push(total)
